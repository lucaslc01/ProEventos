...
namespace ProEventos.API.Controllers
{
    ...
    public class LotesController : ControllerBase
    {
        ...
        [HttpGet("{eventoId}")]
        public async Task<IActionResult> Get(int eventoId) //o IActionResult retorna um objeto de resposta contendo o número do erro de requisição http
        {
            try
            {
        ...
        public async Task<IActionResult> Delete(int eventoId, int loteId){
            try
            {
                ...
                return await _loteService.DeleteLote(lote.EventoId, lote.Id) ? Ok(new { message = "Deletado" } ) : BadRequest("Lote não deletado");
            }
            catch (Exception ex)
            {
